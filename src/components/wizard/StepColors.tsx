...
export function StepColors({ colors, format, dispatch, onNext }: StepColorsProps) {
  const t = useT()
    ...
    <div className="flex flex-col items-center gap-8 py-8">
      ...
      <div className="flex items-center gap-5">
        {ALL_COLORS.map((color) => {
          ...
          const isMaybe = colors[color] === 'maybe'
          return (
            <button
              key={color}
              type="button"
            ...
            >
              <ManaSymbol
                ...
                selected={isSelected}
              />
              <span className={`text-xs ${isSelected ? 'text-surface-100 font-medium' : 'text-surface-500'}`}>
                {t(COLOR_KEYS[color])}
              </span>
            </button>
          )
